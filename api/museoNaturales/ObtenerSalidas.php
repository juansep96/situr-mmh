...
$stmt = $conn->prepare("SELECT COUNT(*) AS allcount FROM museoNaturales_salidas left join instituciones ON idInstitucion_salida = id_institucion  WHERE estado_salida=1 ");
$stmt->execute();
$records = $stmt->fetch();
$totalRecords = $records['allcount'];

## Total number of records with filtering
$stmt = $conn->prepare("SELECT COUNT(*) AS allcount FROM museoNaturales_salidas left join instituciones ON idInstitucion_salida = id_institucion WHERE 1 ".$searchQuery ." AND estado_salida=1  AND fecha_salida BETWEEN '$fechaInicio' AND '$fechaFin'");
$stmt->execute($searchArray);
$records = $stmt->fetch();
$totalRecordwithFilter = $records['allcount'];

## Fetch records
$stmt = $conn->prepare("SELECT * FROM museoNaturales_salidas left join instituciones ON idInstitucion_salida = id_institucion WHERE 1 ".$searchQuery ." AND estado_salida=1  AND fecha_salida BETWEEN '$fechaInicio' AND '$fechaFin' ORDER BY ".$columnName." ".$columnSortOrder." LIMIT :limit,:offset");
