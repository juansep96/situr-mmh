...
## Fetch records
$stmt = $conn->prepare("SELECT * FROM tourist_attentions left join cities on id_city = id_origin_tourist_attention LEFT JOIN provinces ON id_province = id_province_city left join motivosConsulta ON idMotivo_attention = id_motivo LEFT JOIN users on id_user = id_user_tourist_attention WHERE 1 ".$searchQuery ." AND (date_tourist_attentions BETWEEN '$fechaDesde' AND '$fechaHasta') AND active_tourist_attention=1 AND id_dependence_user = '$idDependencia' ORDER BY ".$columnName." ".$columnSortOrder." LIMIT :limit,:offset");
// Bind values
